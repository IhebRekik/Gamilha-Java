...
public class AdminStreamFormController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        viewersSpinner.setValueFactory(new SpinnerValueFactory.IntegerSpinnerValueFactory(0,999999,0));
    ...
    public void initCreate() {
        editing = null;
        pageTitle.setText("➕ Nouveau Stream (Admin)");
        submitBtn.setText("Créer le stream");
        if (obsInfo != null) { obsInfo.setVisible(false); obsInfo.setManaged(false); }
    }

    public void initEdit(Stream s) {
        editing = s;
        pageTitle.setText("✏ Modifier — " + s.getTitle());
        ...
        if (obsInfo != null && s.getStreamKey() != null) {
            obsInfo.setText("🔑 " + s.getStreamKey() + " | 📡 " + (s.getRtmpServer() != null ? s.getRtmpServer() : "N/A"));
    ...
    private void onSubmit(ActionEvent e) {
        if (!(vTitle() & vDesc() & vGame() & vUrl())) return;
        try {
            ...
            s.setUrl(urlField.getText().trim());
            s.setThumbnail(thumbField.getText().trim());
            s.setViewers(viewersSpinner.getValue());
            s.setStatus(statusCombo.getValue());
            s.setIsLive("live".equals(statusCombo.getValue()));

            if (editing == null) { s.setUserId(1); service.create(s); AlertUtil.showSuccess("✅ Créé !", "Stream créé."); }
            else                 { service.update(s); AlertUtil.showSuccess("✅ Modifié !", "Stream mis à jour."); }

            MainApp.loadScene("admin/AdminStreamList.fxml");
        } catch (SQLException ex) { AlertUtil.showError("Erreur BDD", ex.getMessage()); }
    }

    @FXML private void onCancel(ActionEvent e) { MainApp.loadScene("admin/AdminStreamList.fxml"); }

    private boolean vTitle() { String err=ValidationUtil.validateTitle(titleField.getText()); ValidationUtil.mark(titleField,err); ValidationUtil.setErr(errTitle,err); return err==null; }
    private boolean vDesc()  { String err=ValidationUtil.validateDescription(descArea.getText()); ValidationUtil.mark(descArea,err); ValidationUtil.setErr(errDesc,err); return err==null; }
    private boolean vGame()  { String err=ValidationUtil.validateGame(gameCombo.getValue()); ValidationUtil.mark(gameCombo,err); ValidationUtil.setErr(errGame,err); return err==null; }
    private boolean vUrl()   { String err=ValidationUtil.validateUrl(urlField.getText()); ValidationUtil.mark(urlField,err); ValidationUtil.setErr(errUrl,err); return err==null; }
}
