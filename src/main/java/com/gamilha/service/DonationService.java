...
/**
 * DonationService — couche service pour les opérations sur Donation.
 * Renommé depuis DonationDAO.
 */
public class DonationService {
    ...
    public boolean delete(int id) throws SQLException {
        try (PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("DELETE FROM donation WHERE id=?")) {
            ps.setInt(1, id);
            return ps.executeUpdate() > 0;
        }
    }
