...
public class InscriptionAdminController {
    ...
    @FXML
    private ComboBox<String> sortDate;
    ...
    public void initialize(){
        ...
        filterType.getItems().addAll(
                masterData.stream()
                        .map(i -> i.getAbonnements().getType())
                        .distinct()
                        .toList()
        );
        ...
        sortDate.getItems().addAll(
                "Défaut",
                "Date début croissante",
                "Date début décroissante",
                "Date fin croissante",
                "Date fin décroissante"
        );

        sortDate.setValue("Défaut");

        buildTable(masterData);

        filterType.valueProperty().addListener((obs,o,n)->filter());
        sortDate.valueProperty().addListener((obs,o,n)->filter());
        dateDebutPicker.valueProperty().addListener((obs,o,n)->filter());
        dateFinPicker.valueProperty().addListener((obs,o,n)->filter());
    ...
    private void filter(){
        ...
        ObservableList<Inscription> filtered =
        ...
        if("Date début croissante".equals(sort))

            filtered.sort(
                    (a,b)->a.getDateDebut()
                            .compareTo(b.getDateDebut())
            );


        if("Date début décroissante".equals(sort))

            filtered.sort(
                    (a,b)->b.getDateDebut()
                            .compareTo(a.getDateDebut())
            );


        if("Date fin croissante".equals(sort))

            filtered.sort(
                    (a,b)->a.getDateFin()
                            .compareTo(b.getDateFin())
            );


        if("Date fin décroissante".equals(sort))

            filtered.sort(
                    (a,b)->b.getDateFin()
                            .compareTo(a.getDateFin())
            );
