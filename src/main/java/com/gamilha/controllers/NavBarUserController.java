...
public class NavBarUserController implements Initializable {
    ...
    private void injectUser(Object ctrl) {
        ...
        if (ctrl instanceof UserBookmarksController c)       c.setCurrentUser(u);
    ...
    private void setSubActive(Button target) {
        ...
        Button[] subs = {btnFilActualite, btnMesPosts, btnMesCommentaires, btnListAmis, btnFavoris};
        for (Button b : subs) {
            if (b == null) continue;
    ...
    void gofav() {
        ...
        load(BASE_USER + "FavoritesList.fxml");
    }
    @FXML
    void gonot() {
        ...
        load(BASE_USER + "NotificationList.fxml");
    }
    @FXML
    void govd() {
    ...
    @FXML void goFavoris()         { setSubActive(btnFavoris);         load(BASE_USER + "UserBookmarksView.fxml"); }
