...
public class DBconnexion {
    ...
    public String url = System.getenv().getOrDefault(
            "GEVENT_DB_URL",
            "jdbc:mysql://localhost:3306/defaultdb1?useSSL=false&serverTimezone=UTC&allowPublicKeyRetrieval=true"
    );
    ...
    private DBconnexion() {
        try {
            ...
            throw new RuntimeException("Connexion DB impossible: " + e.getMessage(), e);
        }
    }
    ...
    public Connection getCnx() {
        if (cnx == null) {
            throw new IllegalStateException("Connexion JDBC non initialisee.");
        }
        return cnx;
    }
