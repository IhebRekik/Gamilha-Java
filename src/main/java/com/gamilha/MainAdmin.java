...
/**
 * ══════════════════════════════════════════════════════
 *  TEST — Interface ADMIN  (Coaching & Playlist)
 *  Charge directement PlaylistList.fxml (lecture seule)
...
public class MainAdmin extends Application {
    ...
    public void start(Stage stage) throws Exception {
        ...
        // ── 1. Utilisateur Admin fictif ─────────────────────────────────────
        User admin = new User();
        admin.setId(1);
        admin.setName("Admin Test");
        admin.setEmail("admin@gamilha.com");
        admin.setRoles(String.valueOf(List.of("ROLE_ADMIN")));
        admin.setActive(true);
        admin.setCreatedAt(Timestamp.valueOf(LocalDateTime.now()));
        SessionContext.setCurrentUser(admin);

        // ── 2. Barre de navigation minimale (pour switcher entre Playlist / Vidéo) ──
        Label logo = new Label("🛡 Admin — Coaching");
        logo.setStyle("-fx-text-fill:#c084fc;-fx-font-size:16;-fx-font-weight:bold;");

        Label badge = new Label("👁 Mode lecture seule");
        badge.setStyle("-fx-text-fill:#64748b;-fx-font-size:12;-fx-background-color:#1e293b;"
        ...
        stage.setTitle("🎮 Gamilha Admin — Coaching");
