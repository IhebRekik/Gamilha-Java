...
public class StreamService {
    ...
    public Stream findById(int id) throws SQLException {
        try (PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("SELECT * FROM stream WHERE id=?")) {
            ps.setInt(1, id);
    ...
    public boolean delete(int id) throws SQLException {
        try (PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("DELETE FROM stream WHERE id=?")) {
            ps.setInt(1, id);
            return ps.executeUpdate() > 0;
        }
    }
