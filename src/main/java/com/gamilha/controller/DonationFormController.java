...
public class DonationFormController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        amtSpinner.setValueFactory(new SpinnerValueFactory.DoubleSpinnerValueFactory(0.5,9999.0,1.0,0.5));
        ...
        catch (Exception ex) { AlertUtil.showError("Erreur", "Impossible de charger les streams."); }
        updateEmoji(1.0);
    }

    public void initCreate(Stream s) {
        editing = null; ctxStream = s;
        pageTitle.setText("💰 Nouvelle Donation");
    ...
    public void initEdit(Donation d, Stream s) {
        ...
        pageTitle.setText("✏ Modifier la Donation");
    ...
    private void onSubmit(ActionEvent e) {
        ...
        try {
            ...
            if (editing == null) { dService.create(d); AlertUtil.showSuccess("✅ Enregistrée !", "Donation de " + d.getFormattedAmount() + " créée."); }
            else                 { dService.update(d); AlertUtil.showSuccess("✅ Modifiée !", "Donation mise à jour."); }
            ...
            } else MainApp.loadScene("StreamList.fxml");
    ...
    @FXML private void onCancel(ActionEvent e) {
        Stream s = ctxStream != null ? ctxStream : streamCombo.getValue();
        if (s != null) { DonationListController c = MainApp.loadSceneWithController("DonationList.fxml"); if (c != null) c.setStream(s); }
        else MainApp.loadScene("StreamList.fxml");
    }

    private boolean vDonor()  { String err = ValidationUtil.validateDonorName(donorField.getText()); ValidationUtil.mark(donorField,err); ValidationUtil.setErr(errDonor,err); return err==null; }
    private boolean vAmt()    { String err = ValidationUtil.validateAmount(amtSpinner.getValue());  ValidationUtil.mark(amtSpinner,err); ValidationUtil.setErr(errAmt,err);   return err==null; }
    private boolean vStream() { String err = ValidationUtil.validateStreamSelected(streamCombo.getValue()); ValidationUtil.mark(streamCombo,err); ValidationUtil.setErr(errStream,err); return err==null; }
}
