...
public class StreamListController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        sortFilter.setItems(FXCollections.observableArrayList("Plus récents","Plus regardés"));
        sortFilter.setValue("Plus récents");

        // Recherche + tri en temps réel (pas besoin de cliquer un bouton)
    ...
    // ── Construction du FlowPane de cards ─────────────────────────────────
    ...
    // ── Card visuelle (équivalent des cards stream/index.html.twig) ────────
    private VBox buildCard(Stream stream) {
        ...
        btnDonate.setStyle("-fx-background-color:#1e293b;-fx-text-fill:#e2e8f0;-fx-border-color:#334155;-fx-border-radius:8;-fx-background-radius:8;-fx-cursor:hand;-fx-padding:6 14 6 14;");
        btnDonate.setOnAction(e -> {
            DonationFormController c = MainApp.loadSceneWithController("DonationForm.fxml");
            if (c != null) c.initCreate(stream);
        });

        btns.getChildren().addAll(btnWatch, btnDonate);
    ...
    @FXML private void onRefresh(ActionEvent e) {
        searchField.clear(); gameFilter.setValue("Tous les jeux"); sortFilter.setValue("Plus récents");
        load();
    }
